...
export async function installIntegrations(config, projectPath) {
  ...
  for (const tool of integrations) {
    ...
    // XSPEC-418 R2: honors config.integrationTargets (set from --claude-target at
    // init time) rather than always assuming each tool's default file.
    const targetFile = resolveIntegrationTargetFile(tool, { integrationTargets: config.integrationTargets });
    ...
    const enhancedConfig = {
      ...
      outputLanguage,
      // XSPEC-418 R2: forwarded so writeIntegrationFile resolves the actual target.
...
export async function generateClaudeMd(config, projectPath) {
  ...
  // Check if Claude Code is selected and its target file doesn't already exist
  // XSPEC-418 R2: the target may be CLAUDE.local.md, not CLAUDE.md.
  const claudeCodeSelected = aiTools.includes('claude-code');
  if (!claudeCodeSelected || integrationFileExists('claude-code', projectPath, { integrationTargets: config.integrationTargets })) {
