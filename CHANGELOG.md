...
- **`uds init` wrote a pre-commit hook (`.husky/pre-commit`, or `.git/hooks/pre-commit` for non-Node projects) without ever confirming git would actually run it.** It relied on husky's own bootstrap — `npm install` triggering husky's `prepare` script to set `core.hooksPath` — which only fires the NEXT time `npm install` runs; if `node_modules` already existed, that never happened. Verified against three real adopters (asiaostrich-telemetry-server, asiaostrich-telemetry-client, machine-setup, 2026-09-26): all three have a `.husky/pre-commit` calling `npx uds check`, none has `core.hooksPath` set, and the check has never once run on commit — silently, with no error. `uds init` no longer installs husky or touches package.json's dependencies for the adopter; it wires `git config --local core.hooksPath .husky` directly instead (the same thing husky's own bootstrap does internally), so the check is live immediately after `uds init`, with or without husky ever being installed. Never overrides an adopter's own `core.hooksPath`, or an existing `.git/hooks/pre-commit` — both are left untouched with a message explaining the check is NOT enabled and why. The non-Node native-hook path no longer unconditionally overwrites an existing `.git/hooks/pre-commit` either (it used to). New `uds check` warning `[pre-commit]` detects a UDS-managed hook file that exists but is not on git's actual execution path (covering direct wiring, husky's own `<dir>/_` shim forwarding, and the native default) and reports the fix — this is how an adopter who already has the defect (like the three repos above) finds out. Advisory only; does not affect `uds check --ci`'s exit code, since this is a per-clone local-config gap, not a standards-compliance defect. `core.hooksPath` is never committed, so this wiring only applies to the clone that ran `uds init` — a message and the new `uds check` warning both say so.

- **Follow-up (2026-09-27): the pre-commit fix above could make an adopter's EXISTING `.husky/pre-commit` fail every commit, if that file still used husky v8's `_/husky.sh` sourcing line.** Verified against a real disposable clone of one of the three adopters above: running `uds check`'s own suggested fix (`git config --local core.hooksPath .husky`) on a legacy template produced `.husky/pre-commit: line 2: .husky/_/husky.sh: No such file or directory` and `git commit` exited 1 — worse than the original silent-skip defect, because that `.husky/_/` shim directory only exists after husky's own bootstrap has actually run, and setting `core.hooksPath` to `.husky` directly makes git execute the file as-is. `uds init`'s `setupHuskyHook` now detects and removes that line when rewriting `.husky/pre-commit` (preserving everything else — the adopter's own added commands, an existing `uds check` line); `uds check`'s `[pre-commit]` warning no longer suggests the hooksPath fix in isolation when it detects the legacy line — it gives the two-step fix (delete the line, then set hooksPath) instead, since `uds init` refuses to run at all on an already-initialized project and cannot fix this for the three real adopters found. New shared helpers `hasLegacyHuskyShLine`/`stripLegacyHuskyShLine` in `git-hooks.js`.

## [6.13.0-beta.2] - 2026-09-26

> **Pre-release** — install with `npm install -g universal-dev-standards@beta`. What to test, known limitations and how to go back to stable: [docs/PRE-RELEASE.md](docs/PRE-RELEASE.md).
...
- **The Codex adapter's R9 exemption ("the human asked the turn to end") never actually fired against a real Codex install — `bestEffortLastUserMessage()` in `scripts/hooks/check-turn-completion-codex.mjs` read neither of the two record shapes it tried.** Verified against a real codex-cli 0.156.1 `~/.codex/sessions/**/*.jsonl` transcript: a user message record is `{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":...}]}}` — the message lives under `payload`, not at the record's top level and not under a `message` key, so the field was always read as absent and R9 could never exempt a Codex turn. Now reads `payload.type === "message"` first (falling back to the two original shapes for any other record type that might use them), extracts `input_text` content items alongside the existing `text` shape, and does not treat a `role: "developer"` record as a user message. `core/turn-completion-integrity.md`'s "Supported harnesses" section and `docs/PRE-RELEASE.md` updated from "not confirmed against a real install" to the confirmed shape.
- **R9's zh-TW stop-request detector missed "leaving now, resume later" phrasing — measured live: "我要出門了，等我回來再繼續" ("I'm heading out, let's continue when I'm back") really did misfire a Claude Code turn-completion block on 2026-09-25.** Neither that sentence nor "暫停，我要出門" ("pause, I'm heading out") matched any existing `STOP_REQUEST` alternative. Added two narrow alternatives requiring a leaving word (出門/離開一下/先走) to co-occur with either a "resume later" word (等我回來/回來再/明天再/晚點再/待會再) or the bare word 暫停 in the same short span — a leaving word alone ("出門前把這三件做完", an instruction to finish something BEFORE leaving) still does not exempt, per corpus. While auditing the English pack for the same gap, found `\bI'?m (heading|going) (home|out)\b` was unreachable dead code: `isStopRequest()` always calls `normalize()` first, which rewrites "I'm" to "I am", so the contraction-only pattern could never match; fixed to `I(?:'m| am) (heading|going) (home|out)`, verified false before / true after on a bare "I'm heading out."
...
- **`turn-completion-integrity` 1.4.0 extends the Stop hook to Codex and Gemini CLI, alongside the existing Claude Code adapter.** Judgement (locale packs, cooldown, rolling window, the self-echo marker) moved into a shared `turn-completion/engine.mjs` so the three per-harness scripts translate their tool's own contract instead of each carrying a copy of the detection logic. Codex (`.codex/hooks.json`, Stop event) reads `last_assistant_message` directly from stdin and blocks via `{"decision":"block","reason":...}` on stdout at exit 0 — its docs state plain text or empty stdout is invalid there, unlike Claude Code where silence allows; the human's last message is read best-effort from `transcript_path` since its exact schema was not confirmed against a real install, so R9 (exempting a human-directed stop) is a documented gap on Codex, not silent failure. Gemini CLI (`.gemini/settings.json`, `AfterAgent` event) gets both `prompt` and `prompt_response` directly on stdin — no transcript parsing needed — and blocks via `{"decision":"deny","reason":...}`, the documented "preferred for all logic" path over exit code 2. `uds init --with-hooks` now also calls `installCodexHooks`/`installGeminiHooks`, gated on whether the adopter selected that tool, so nothing is written into a project that doesn't use Codex or Gemini CLI. Cursor was evaluated and is explicitly named as not supported (unresolved whether its stop hook can actually block a turn). See [Supported harnesses](core/turn-completion-integrity.md#supported-harnesses) and [CLI-INIT-OPTIONS.md](docs/CLI-INIT-OPTIONS.md#enforcement-hooks-beyond-claude-code).
...
- **`turn-completion-integrity`'s zh-TW and en detectors fired on a conditional commitment whose precondition was the human's own decision.** `"你選定後，我會把這一輪的發想寫成正式決策紀錄…"` ("once you decide, I will write up …") was read as an unkept commitment — the existing exemptions only covered a request for information or a request to report back, not a precondition phrased as the human's decision. zh-TW adds a narrow `你 + short decision verb (選定/選好/決定/確認/回覆/點頭) + 後 + comma + 我` pattern; en adds a grammar-based `(once|after|as soon as) you ..., I` pattern, matching this pack's existing no-verb-list design rather than enumerating verbs. Both corpora gained paired counter-examples (subject not 你/you, or only half the shape present) proving the narrowing does not miss a real unkept commitment; the en pack also documents one known limit left uncovered on purpose (no comma between the precondition and "I" still fires, since widening past that risks missing real commitments).
...
- **`uds update --plan`/`--apply` never converged for integration files: `Migrate Block: N` kept showing even immediately after `--apply`, on a file byte-for-byte what generation would produce.** `diffIntegrations` unconditionally produced a `migrate_block` action for any integration file with UDS markers — "we always update integrations since content is generated dynamically" — because the desired state never carried a hash to compare against (`hash: null`, "computed after generation", and then never actually computed). The desired-state calculator now generates the integration content the same way `--apply` itself would (`buildToolIntegrationConfig` + `generateIntegrationContent`) and hashes just the UDS block, so `diffIntegrations` can tell "content already matches" from "content differs" the same way every other category already does, and reports `unchanged` instead of `migrate_block` when they match. A file whose block genuinely is stale (hand-edited, or generated by an older CLI) still produces `migrate_block` as before. Generation failing for any reason (e.g. a tool the registry cannot resolve) falls back to the old unconditional behavior rather than crashing the plan.
- **`uds check` could print two contradictory "referenced" claims about the same integration file in one run.** `standardsReferenced` ("{count}/{total} standards referenced") scans the whole integration file body — every place a standard's name is mentioned. `standardsNotReferenced` ("Standards not referenced (optional):") only looks at structured `Reference:`/`參考:` lines. A standard mentioned in the file's prose but never listed on a `Reference:` line satisfied the first claim and failed the second in the same run — same English/Chinese word ("referenced"/"參考"), two different measurements, read as a contradiction. Renamed the second message in all three locales to say precisely what it checked (absence from a `Reference:`/`參考:` line) instead of the blanket "not referenced" claim.
- **A bare `uds update --plan` and `uds check` never said Skills or Commands were behind the latest UDS release.** Version staleness was only ever computed by `uds update --plan --skills`/`--plan --commands` — the general reconciliation `--plan` (no scope flag) called a different function that never touched it, and `uds check` had no equivalent at all. An adopter running plain `--plan` or `check` saw a clean report with Skills a full minor behind and no hint that a scoped plan, or `uds update --skills`, had anything to do. Both now print a short `<tool> (<location>): vOLD → vNEW` note when any installed Skills or Commands version is behind, alongside the command that fixes it. Modeled on the existing top-level "Version: X → Y ⚠" row rather than on the integration-block integrity check (XSPEC-418 R1): being behind the latest release is ambient, expected state until the adopter updates, not a compliance defect the way a modified/missing UDS block is — so it does not fail `uds check --ci`.
- **The 6.10.0 dead-link rewrite (P2) deleted a reference written against a pre-6.0.0 filename instead of rewriting it, and could leave a dangling comma behind.** `resolveStandardReferences` compared a `Reference:` line's stem only against the manifest's installed list — never against `STANDARD_ID_MAPPING` (conversion-rules.js), the same old-filename→current-id table `yaml-generator.js` already used elsewhere. A line reading `.standards/commit-message-guide.md` on a project that installed `commit-message` looked exactly like a retired, UDS-owned standard the project chose not to install, and was deleted instead of rewritten to `.standards/commit-message.ai.yaml`. Separately, the comma cleanup after a deletion only handled a trailing comma, a doubled comma, or a stray space before one — not a comma left immediately after the colon when the FIRST reference on the line was the one removed (`Reference:, .standards/other.ai.yaml`). Rebuilt as split → resolve-or-drop → join instead of patching punctuation around a deleted substring.
- **A primary standard reference deleted by the 6.10.0 `resolveStandardReferences` bug did not come back on its own, even after the fix above.** Rewriting away the dangling comma the deletion left behind restores the LINE's punctuation, not the deleted item itself — a UDS rule-template section (e.g. "## 提交訊息標準") lives outside the UDS markers, so nothing regenerates its content once an item is gone. Added a narrow, deliberately scoped auto-repair for this one situation: when a line's heading matches a UDS rule-template heading EXACTLY (any language `RULE_TEMPLATES` ships), and that template's PRIMARY reference (the first `.standards/...` item on its own `Reference:`/`參考:` line) is missing from the file's line, and that standard is genuinely installed, it is added back at the front — everything else on the line (the project's own entries, options files, existing order) is left untouched, no secondary template item is ever restored, and a user-authored section (any other heading) is never touched. Idempotent. Fixes only what 6.10.0's deletion caused; does not attempt to repair references corrupted by unrelated hand edits.
- **A plain `uds update` never repaired a broken `integrationConfigs[file].categories` left behind by the Q1 bug above — only `--sync-refs` did.** A manifest already carrying `categories: []` (from an older buggy `--sync-refs` run) stayed broken through every subsequent plain `uds update --yes`: that command's own integration-sync step writes `integrationBlockHashes` but never even reads `integrationConfigs`, so nothing there ever corrected it — and the next `uds check --restore-missing` then rebuilt the integration file from that broken stored config, silently dropping sections again. `uds update` now repairs an empty or unrecognized `categories` value the same way `--sync-refs` does (`calculateCategoriesFromStandards`), and leaves an already-valid list untouched so a plain update does not turn into a full re-sync every run.
- **`uds update --sync-refs`/`--integrations-only`/`uds check --migrate`/`uds config` could silently downgrade `manifest.version`, and the same sync path computed an empty category set on a normal 3.4.0 manifest.** Five write sites hardcoded an old schema-version literal (`'3.1.0'`/`'3.2.0'`/`'3.3.0'`) instead of the CLI's current schema version (`3.4.0`) — running any of those commands on an already-current manifest wrote the version backwards. Separately, `calculateCategoriesFromStandards` (the function `--sync-refs` uses to decide which sections belong in CLAUDE.md/AGENTS.md) looked up each standard by filename, but a 3.4.0 manifest stores bare stems (`commit-message`, not `commit-message.ai.yaml`) — every lookup missed, the category set came back empty, and the next `--sync-refs` (or a `--restore-missing` rebuild) regenerated the file missing its anti-hallucination/commit-message/code-review sections entirely. Both are now fixed at their single point of truth: a shared `bumpManifestVersion` helper that only ever moves a manifest's version toward current, never backward, and the same stem-aware category lookup (`categoryForStandard`) already used elsewhere in the same file. This restores category calculation to how `--sync-refs` derived it before 6.10.0 — computed from the standards actually installed, not a fixed list — so a full regeneration (`--sync-refs`, `--integrations-only`, or a plain `uds update`) now writes every section that has a rule template and an installed standard behind it (currently 9), not only the 3 sections a fresh `uds init` seeds; a plain block update for a project that already has its full set of sections is unaffected.
- **A sentence merely mentioning the UDS marker text could be mistaken for the real block boundary, and everything between the mention and the real END marker — including the adopter's own content — was deleted (v3.5.0–6.10.0).** Every marker-locating call site used `content.indexOf('<!-- UDS:STANDARDS:START -->')`/`.indexOf('...:END -->')` directly, which matches the marker text anywhere in the file: a sentence explaining the marker syntax by quoting it verbatim (this project's own CLAUDE.md does exactly that), or a fenced code sample containing it, was indistinguishable from the real boundary. This affected all four write paths that regenerate an integration file's UDS block: `uds update --integrations-only`, `uds update --apply`'s `migrate_block` reconciler path, `uds update --sync-refs`, and `uds check --restore` (which XSPEC-418 R6 extended to also restore a damaged UDS block). A marker now only counts when, after trimming, it occupies an entire line by itself and that line is not inside a fenced code block (``` / ~~~); every call site goes through one shared `locateMarkerBlock` helper, enforced by a static-scan guard test so a future call site cannot regress back to raw `indexOf`/`includes`. A file with two genuine marker pairs (corrupted, or a hand-edit gone wrong) is no longer guessed at — every write path refuses and reports both marker lines by number, and `uds check` reports it explicitly instead of folding it into "modified" or "no markers found".
...
- **Integration files (CLAUDE.md, CLAUDE.local.md, AGENTS.md, etc.) were tracked by both their UDS block AND their whole-file content, and the two checks could contradict each other in the same run.** A fresh `uds init` never recorded a whole-file hash for these files, but `uds update --integrations-only` did — after which ANY edit outside the UDS block (exactly the customization the marker-based update is supposed to preserve) made `uds check --ci` report `CLAUDE.md (modified)` and exit 1, in the same run its own block-integrity check said the block was intact. Separately, `uds check --restore` correctly regenerated a damaged block (preserving content outside it, no data lost) but only refreshed the whole-file hash, never the block hash — so the very next check reported `CLAUDE.md (UDS block modified)` for a block that had just been correctly restored. Integration files are no longer added to `fileHashes` (only to `integrationBlockHashes`, which already tracks just the block) — walked every write site, not just the two originally reported, including the `--apply`/`--plan` reconciler path and `--sync-refs`; an existing manifest with a stale whole-file entry for one of these files has it removed on the next `uds update` (or `uds check --restore`/`--migrate`), keeping the block hash. `uds check --restore` on an integration file now updates the block hash to match what it actually wrote. Verified against a real adopter manifest carrying this exact stale entry: `uds check --ci` went from exit 1 to exit 0 with the fix, on identical input. Two gaps found by follow-up review are also closed: `uds check --restore` used to do nothing at all for a damaged UDS block — it is driven by `fileStatus`, built entirely from `fileHashes`, which no longer contains integration files, so it silently restored zero files while the block sat untouched; `--restore` now also regenerates a block reported as modified or missing its markers (unaffected content outside the block, and `--restore-missing` behave as before). And because the new pruning above only runs on a write, an existing manifest with a stale whole-file entry stayed red on `uds check --ci` forever — a plain, read-only `check` never had a chance to clean it up; the standards-file integrity check now also skips, at read time, any `fileHashes` key that is also tracked by `integrationBlockHashes`, so an unmodified project reports clean without needing an `update` first. (XSPEC-418 R6)
...
- **Integration files pointed at standards that are not on disk, and the reference check called them synced.** On a project installed with `--format ai`, `uds update --integrations-only` wrote `Reference: .standards/anti-hallucination.md` and `.standards/checkin-standards.md`; both files exist only in the `human` format. The rule templates spell their references as `.md` regardless of the installed format, and because the rule sections sit outside the UDS markers, a marker-based update never refreshed them — a file first written by an older CLI kept `.standards/commit-message-guide.md` two majors after that file stopped shipping. References are now resolved against the standards this project actually installed, in the format it installed them, both when generating and when updating an existing file; a reference to a standard the project did not adopt is dropped rather than left dangling. `uds check` reports references whose file is missing instead of matching on the name with the extension stripped. Only standards UDS itself ships can be removed: a reference the project added to a file of its own under `.standards/` is left exactly as written.
- **`uds check` recommended `uds update --sync-refs`, which could not run.** `--sync-refs` aborts when the manifest has no `integrationConfigs`, and that key is written only by `uds init`'s interactive flow — a project created with `uds init -y` has `{}` from the first day, so a brand-new 6.9.0 project was already in the state the error message blames on old versions and manual copying. `--sync-refs` now rebuilds the config from the manifest (integrations, AI tools, standards, options) instead of refusing, and `uds check` only names `--sync-refs` when it can actually run; otherwise it points at `uds update --integrations-only`.
- **`uds check` reported an adopted standard as missing from the index.** The id-to-filename map was built from `source.ai` only, but some registry entries store `source` as a string — `zh-tw-locale` is `extensions/locales/zh-tw.md` — so its installed file, `.standards/zh-tw.md`, was never recognised and the count read "67/68 標準已參考, 缺少: zh-tw-locale" on a file that lists it.
- **Reference sync reported option files as orphans and suggested filenames UDS no longer ships.** Options are recorded in `manifest.options`, not `manifest.standards`, so every `.standards/options/*.ai.yaml` reference was reported as "not in manifest"; and the "standards not referenced" list was built from a hand-written table of pre-6.0.0 names (`git-workflow.md`, `error-code-standards.md`, `project-structure.md`). Options are now recognised, and the list names only files this project actually holds.
- **`uds check` listed `AGENTS.md` twice.** Codex and OpenCode share one file; the check iterated tools rather than files. It now reports each file once and names the tools that share it.
- **Seven `core/*.md` standards have not been installed since 6.0.0, and none of them said so ([#180](https://github.com/AsiaOstrich/universal-dev-standards/issues/180)).** Their `.ai.yaml` counterparts were removed in 6.0.0 and the documents were deliberately kept under `core/` as references for the adoption layer — a reasonable decision, recorded in `docs/MIGRATION-v6.md` §2 and in `scripts/reference-only-standards.json`, and stated nowhere a reader of the document itself would meet it. A migration guide is read once at upgrade time; `core/` is read continuously, and anyone walking it to answer "what standards does UDS have" counted 152 files, seven of which `uds init` has never installed. Each of the seven now opens with a `<!-- UDS:REFERENCE-ONLY -->` notice, in English, zh-TW and zh-CN, pointing at the migration record and at the machine-readable list. A new gate (`npm run check:reference-only`, wired into CI with both self-test arms) derives the shipped surface from every `source.human` path in the registry, treats the difference against `core/*.md` as the reference-only set, and requires each member to disclose it — and equally requires a document that ships to *not* carry the notice, so a standard that starts shipping again cannot leave a lie behind. Deriving the set instead of checking seven names is what keeps the next descoping from repeating the same silence.
- **`--format human` installs delivered `testing-standards.md` under two standard ids and never delivered the full-coverage document.** The `full-coverage-testing` registry entry pointed `source.human` at `core/testing-standards.md`, so `core/full-coverage-testing.md` — a live standard, maintained as recently as XSPEC-288 — was installed by nobody, and a human-format manifest listed the same path twice. `check-registry-completeness.ts` Check 2 could not see it: it accepts an entry when *either* the human or the ai path appears anywhere in the registry text, and the ai path was there. Found by the reference-only gate above on its first run, as an eighth orphan among the seven expected. Verified with a real `uds init --format human` install before and after.
- **`uds update --sync-refs` wrote a standards count that `uds check` then contradicted.** On a project whose manifest held 73 standards, `--sync-refs` rewrote CLAUDE.md to announce 76, and `uds check` replied "index declares 76 standards, manifest actually has 73"; `--integrations-only` produced the right number, which made it look like a counting bug rather than a sourcing one. `--sync-refs` regenerated the file from `integrationConfigs[file]`, whose `installedStandards` is a snapshot taken at install time and refreshed by nothing — so on a project upgraded across majors it still listed standards that stopped shipping in 6.0.0, and whatever it listed became the file's content. It now regenerates from the manifest, as `buildToolIntegrationConfig`'s own docblock has said it must; the stored snapshot only still decides `outputLanguage`, so a manifest that predates that option cannot silently reset it.
- **The installed-standards index said "options 0" on every project.** `uds init` reduced the installed list to basenames, and everything downstream classifies an option by an `/options/` segment — with `contentLayout: flat` there is no directory left to recover it from. A project with 66 core standards and 7 options was told it had "73 (73 core, 0 options)". The same reduction sent every option file in AGENTS.md to `.standards/<name>.ai.yaml`, one directory above where option files install, so each one was a dead link under a heading telling the agent to read them. Paths now arrive intact, and the AGENTS.md summary writes `.standards/options/` for options and honours the project's format instead of assuming `ai` (a `--format human` project was being handed `.ai.yaml` names for files it holds as `.md`).
- **Following the migration guide made `uds check` report faults, and the fix it offered could not work.** MIGRATION-v6 §2 said to delete the seven descoped `.ai.yaml` copies by hand. Doing so left their `fileHashes` records behind, so `uds check` reported seven files as missing and recommended `uds check --restore`, which then failed on every one with "could not determine source" — the source has not existed upstream since 6.0.0. The tool asked the adopter to undo its own documentation using a command it already knew could not succeed. A missing file whose name UDS no longer ships anywhere is now reported apart from missing files, in grey, with the remedy that works; `uds update` drops those records and says which, including on a project already on the latest version, which is where every adopter who followed the guide necessarily is. `--prune` does not reach them: it deletes what it finds while walking `.standards/`, and these are precisely the files that are no longer there.
- **UDS regenerated AGENTS.md and did not record what it had written.** `uds update --integrations-only` followed immediately by `uds check` reported `AGENTS.md (modified)` on a file nothing but UDS had touched. All three call sites that write AGENTS.md recorded `integrationBlockHashes` and none recorded `fileHashes`. A record that already exists is now refreshed; a file the manifest does not track is still not enrolled, because that summary is one adopters extend.
- **On Windows PowerShell, a successful run looked like a failure.** Progress lines went to stderr, where PowerShell 5.1 wraps any output from a native command in a `NativeCommandError` — including `✔ Regenerated 2 integration files`, the line saying it had worked. Measured before the change: 9 lines on stdout, 2 on stderr, both of them the spinner. All 47 spinners now report on stdout; stderr is empty on a successful run.
- **Deletion plans gave one reason for three different situations.** "no longer in desired state" was printed for a standard UDS retired upstream, an option the project deselected, and a path the desired state does not model — three different right responses. The reason now says which, and whether re-selecting would bring the file back. `.standards/release-config.yaml` is no longer a deletion candidate at all: `uds init` writes it from the release mode the user picked and `uds config` rewrites it, so a plan that deleted it was discarding a setting and calling it reconciliation. It is kept with the reason stated, not skipped silently.
- **A .NET Framework project was handed four commands, three of which cannot run.** `.csproj` has named two incompatible build systems since 2017, and the detector keyed on the extension alone: an old-style project (`<TargetFrameworkVersion>`, no `Sdk` attribute) got `dotnet build`, which fails with `MSB4019`, and `dotnet list package --vulnerable`, which silently reports nothing for `packages.config` because it reads `PackageReference` only. Old-style projects now get `msbuild` for `build` and blanks elsewhere — the unknown-ecosystem branch already returns blanks "so user fills them in", and a blank a human completes beats a command that looks authoritative and fails on first use.
- **Every generated instruction file opened by telling the agent to read a directory that is never installed.** All nine tools, in every language, began "PRIORITIZE reading the concise rules in `core/` (e.g. `core/testing-standards.md`)". `uds init` installs into `.standards/` and has never created `core/` in an adopter's project — that is where standards live in the UDS repo, not in the repos that adopt it. 36 copies of the same two sentences, all wrong the same way, and a clean 6.9.0 install wrote them. They now name `.standards/`, and a test walks the generated output so a tenth tool cannot reintroduce it.
- **`uds update` rewrote a Chinese CLAUDE.md in English.** `uds init` writes integration content in `display_language`; every regeneration path derived it from `output_language`, the commit-message language, which defaults to `english`. Content language now follows `display_language` and falls back to `output_language` only when there is no display setting to go on.


### Added

- **`uds check` now reports paths an instruction file mentions that do not exist.** Previously it only examined `Reference:`/`參考:` lines and only `.standards/` paths, which is the right scope for reference *sync* — those are the lines UDS generates — and the wrong scope for "does this instruction point at anything". Prose outside the UDS markers, left by an older install and revisited by no regeneration, sent agents to files that are not there. The scan is existence-based rather than shape-based, so a project with its own `core/` directory is not accused of anything, and it ignores paths inside URLs. It found two of the defects fixed in this release on its first run.

## [6.9.0] - 2026-09-14

### Migration notes for adopters

- **`uds update` no longer deletes files automatically.** It lists UDS files that can be removed, deletes them only with `--prune`, and only ever deletes files UDS itself wrote. Workflows that relied on automatic cleanup should add `--prune`. (#168, #165 — details under Fixed.)
- **An unrecognised `--content-mode` value now prints a warning and still uses `index`** (6.8.0 also generated the `index` output for it). From 7.0.0 it will be an error. `--content-mode full` is deprecated and resolves to `index` with a notice. (Details under Deprecated.)
- **Codex, OpenCode and other non-Claude tools installed with `uds init -y` now receive skills in their own directory** (for example `.agents/skills/` for Codex) instead of `.claude/skills/`. Running `uds update` on an existing install migrates it. (Details under Fixed.)
- **Antigravity now gets `.agents/skills/` and `.agents/AGENTS.md`** instead of nothing and `INSTRUCTIONS.md`. Antigravity only discovers `.agents/` once the directory is a registered project; `uds init` prints this.
- **Hooks written into `.claude/settings.json` by `uds init --with-hooks` in 6.8.0 and earlier use a string format that never ran.** `uds update` does not handle hooks, and re-running `uds init --with-hooks` does not remove the old entries or `scripts/hooks/*.js`. To get working hooks, re-run `uds init --with-hooks`; you may delete the old `.js` entries by hand.

### Fixed

- **`uds update` no longer decides ownership by looking in a hash table (XSPEC-384, issues [#165](https://github.com/AsiaOstrich/universal-dev-standards/issues/165) and [#168](https://github.com/AsiaOstrich/universal-dev-standards/issues/168)).** Two users reported opposite defects in one command: it deleted a hand-written project document under `.standards/` with no warning, diff, prompt or backup (#168, found only by diffing `git status` line by line), and it kept standards the registry had dropped, in both the manifest and on disk, while `upstream.version` advanced to "latest" (#165). Both come from one boolean answering two independent questions — *did UDS write this file* and *is this still a shipped standard* — and each issue is one end of the overloaded axis. Ownership is now recorded when UDS writes, in a new manifest `provenance` field keyed by the path the copy actually returned rather than one re-derived from the source name (the re-derivation is what put five `.standards/options/<name>.ai.yaml` entries in `fileHashes` for files that live at `options/<category>/<name>.ai.yaml`). Provenance is append-only where `fileHashes` is current state; a record pruned in step with `fileHashes` would fail at the same moment and add nothing. Deletion now needs both facts to agree, and every other combination is kept **with the reason printed**. Ownership that cannot be determined resolves to keep — the branch the old code declined to take. Existing installs carry no provenance, so the first upgrade attributes nothing and removes nothing; provenance is established only at the end of a run that wrote the full set. Deletion is additionally gated behind a new `--prune`, deliberately not `--yes`: `--yes` is set in CI, and letting it authorise deletion would arm the destructive path exactly where nobody can see it. The staging policy for enabling deletion by default sits in one named constant (`PRUNE_POLICY`) because XSPEC-384 §4 has not decided it. Every removal report prints its denominator — files examined, owned, foreign, unknown, excluded and why — because a report listing only its targets is indistinguishable from a scan that only saw those files. Both issues are pinned by tests proven red first.
- **Skills went to a directory eight of nine tools never read.** In the non-interactive path (`uds init -y`), skills for every skills-capable tool except Claude Code were written into `.claude/skills/`; a Codex repo with `--mode skills -y` got zero skills and no message. The skills-capable set and the marketplace decision now come from the path table, a detected tool with no verified skills path installs nothing and says so, and `uds update` re-derives the agent when converting a legacy manifest instead of assuming `claude-code`. Codex's unverified `fallbackSkillsPath: '.claude/skills/'` is removed. The interactive flow was never affected.
- **Roo Code could never be installed.** It had path-table entries but no detector, and its identifier was `roo` in one table and `roo-code` everywhere else, so `uds init` wrote instructions to `roo-code.md` in the repo root. Detection now uses `.roo/` and `.roorules` (deliberately not `.clinerules`, which is Cline's own marker), and skills no longer fall back to `.claude/skills/`, which Roo Code's docs do not list. Based on Roo Code's documentation; not measured against a live Roo Code install.
- **Antigravity: UDS installed nothing for six months, and the instruction file was one it never reads (XSPEC-355 OQ6).** Skills now go to `.agents/skills/` and instructions to `.agents/AGENTS.md`, confirmed by the tool's binary, its bundled guide, and a live run with a decoy path. Its tier moves from `minimal` to `partial`.
- **Every hook UDS had shipped was inert, and adopters received four broken hooks.** Hook entries were written as bare strings (only the object form runs); `scripts/hooks` was not in the npm package; hooks were ESM `.js`, which fails under `"type": "commonjs"` (now `.mjs`); and the commit-message validator ran on every prompt (`UserPromptSubmit`) and exited 1 each time (now `PreToolUse(Bash)`, inspecting only a real `git commit -m`).
- **`uds check` restore overwrote generated integration files with a stale template.** For Claude Code that was a 140-line file last touched 2026-03-25. Restore now regenerates from the config the installer recorded, leaving content outside the managed block untouched; Antigravity and Gemini CLI gain a restore path, and Codex's fallback no longer points at a directory that does not exist.
- **The Codex/OpenCode `AGENTS.md` carried no disclosure.** The 2026-08-18 disclosure fix reached only one of two generators; selecting codex or opencode routed `AGENTS.md` through the other one. Fixed in both languages and both output formats.
- **cline, windsurf, roo-code and Antigravity integration files pointed at the commit-message rule instead of stating it.** The format and valid types are now written inline, as the Codex file already did.

### Added

- **`turn-completion-integrity` — an agent must not end a turn having stated a next action it did not take.** Enforced at turn end by a Stop hook rather than an instruction. The detector ships with English and zh-TW language packs and a mutation-tested corpus; it reads the human's last message so an instruction to stop is not treated as an abandoned commitment, recognises an itemized "what is blocked on whom" list as a legitimate ending, and treats "tell me when it's done and I will verify" as conditional. `uds init --with-hooks` now warns when your language has no pack, because in an unlisted language the hook installs and can never fire. zh-TW and zh-CN translations included.
- **`deferred-item-exit` — six standards produce deferred items and none of them said where those items go (XSPEC-391).** An adopter's one design session produced an ADR and a spec carrying five items marked "to be decided" / "separate effort" / "not in v1", and **all five entered no tracker at all**. The cause is not forgetfulness. Measured across the six standards that produce such items — `adr-standards`, `spec-driven-development`, `retrospective-standards`, `feature-discovery-standards`, `code-review-checklist`, `acceptance-criteria-traceability` — a grep for `issue|tracker|backlog|task id|work item` returns 13 hits and **not one of them is an exit**: ADR's `Technical Story` is an input reference, retrospective treats a tracker as a metrics source, feature-discovery mines tickets as requirement input, AC-traceability requires a link only for externally-blocked coverage, and all 8 hits in code-review are the English word "issues" meaning problems. Each standard says how to write its document well; the document is then the end of the line. The new standard states the missing relation once — a deferred item has a traceable exit outside the document, and the document carries that exit's identifier — and the six producers plus the `brainstorm-assistant` skill each gain a **one-line pointer to it rather than a copy of it**, because six copies of one rule rot in six directions. **The carrier is deliberately unspecified**: an issue, a tracked TODO, a manifest row, a line in a backlog file all satisfy it, and the test is "has this item left the document", never "which tool". Three findings shape the rules. First, the exit must still resolve *after* the change that produced the document lands — two observed leaks satisfy every other condition and still lose the item (the todo written into an issue the merge closes; the todo written into a comment on the exit, pushed out of view by later comments). Second, **a present link is not a working link**: the reporting adopter's own gate would have passed his strongest example, a document reading "to be decided together with issue #19" where #19 contained nothing about it — so DEX-005/006 require that "linked and content-verified" and "linked, content unverified" never print the same green, since an unknown reported as a pass is worse than an unknown reported as unknown. Third, the anchor is the document's **structure** (ADR's `Consequences`, a spec's out-of-scope section, a retrospective's `Action Items`), not its wording: structure can be enumerated and a wording list cannot, so DEX-008 requires any wording list to declare its coverage unknown, and DEX-009 requires any window size ("within 3 lines") to carry its provenance or be marked uncalibrated. The standard is written entirely as relations that must exist and never as mechanisms that must maintain them, per DEC-049 — and it says plainly, in its own text, that **UDS ships no gate for it**, because a standard that states a rule while nothing executes it is worse when it does not admit that than when it does. `adr-standards` additionally gains an explicit boundary it had never stated (a grep for `acceptance criteria|\bAC\b|驗收` across it returned **zero**, against a control of 84 hits in `acceptance-criteria-traceability`): **acceptance criteria are maintained in exactly one place, the SPEC**, and the ADR links to them — the copied list has two owners, one is never updated, and the measured consequence is an ADR whose checkboxes all read unticked after the work shipped, invisible to coverage tooling that reads only the spec.
- **`AUTH_SCOPE_EXTRACTION` — the tenant boundary is a `WHERE` clause, and nothing in a migration forces you to look for it (`reverse-engineering-standards` 1.3.0, issue [#166](https://github.com/AsiaOstrich/universal-dev-standards/issues/166)).** In a real PHP → C# migration, a tenant-scoped account-listing endpoint lost its isolation predicate during the rewrite: per-role scoping was replaced with a coarse "is this account any enabled administrator?" check, and the endpoint returned every account in the database regardless of which tenant asked. It shipped, it passed every existing test — the method had none — and it surfaced when a customer reported seeing account information that should not have been there. `test-completeness-dimensions` Dimension 4 already names cross-tenant access, so the dimension was never missing; what was missing is a step that forces the recall at migration time. Three properties let this class survive every gate: the security-critical line reads as one filter among several because it lives inside a query rather than in the request/response contract; shape-based assertions cannot see scope, so "200 with a non-empty list" passes identically with the clause deleted; and a shared scope resolver existing elsewhere in the codebase reads as coverage while a new call site reimplements the query without it. The new stage runs alongside Implicit Rule Scan and mechanically derives every predicate bounding results by the operator's own identity — including scope enforced outside SQL by ORM global scopes, repository base classes, RLS policies and filter-injecting middleware, because a grep over SQL alone reports a smaller set than exists and a smaller set here reads exactly like a safer one. Each match is recorded with `file:line`, its self-binding and its triggering branch; the oracle is predicate equivalence on every role branch and per call site, not output shape. RE-AUTH-001 requires a negative test asserting the **absence** of another operator's records (with the self-check: delete the predicate in a scratch copy, and if the test still passes it was never testing scope); RE-AUTH-002 blocks cutover on any predicate whose ported equivalent is unverified.
- **A written-down date now has to mean something (`tech-debt-standards` 1.1.0, `governance-layer` 1.1.0).** The registry template has demanded an `Owner` and a `Target Resolution Date` since 1.0.0, and nowhere in either standard did anything say what happens when that date passes — a grep for `overdue|past due|expired|exceed` across `core/tech-debt-standards.md` returned nothing, while a control grep for `Owner` returned a hit, so the absence was real and not a broken query. Worse, `Registry Storage Options` listed "Dedicated spreadsheet" as fully legal: a table no program can read satisfies every other requirement in the section, and nothing about it is wrong until the date passes and nothing happens. `tech-debt-standards` gains **Overdue Handling** — exactly three legal dispositions on expiry (resolve / withdraw / extend-with-a-written-reason), TD-EXP-001..006, an explicit prohibition on implementing expiry as automatic extension or automatic closure (both stop the clock and neither leaves a trace that they did), and an **Unattended Declaration** as the compliant alternative for teams that cannot run a check. Each storage option now carries the condition under which it is legal; the spreadsheet is deliberately kept, because it is often the only format the people funding the work will open. `governance-layer` gains `review_by` on the Risk Acceptance Clause (an acceptance whose `review_by` has passed is not valid, and the pipeline fails closed as if it were absent), **Pending Decisions** (GOV-PD-001..004 — a pending decision is a governed state carrying a clock and a fixed enumeration marker, not free prose), **Aggregate Reporting** (GOV-RPT-001..005 — passed / failed / **undecidable** printed separately, the denominator and how it was derived stated, no single collapsed score, because a metric on which doing nothing scores the same as doing the work is not measuring the work), and **Freshness Metrics** (GOV-FRESH-001..004 — an indicator must name whether it reads edit time or reconciliation time, and the two may not share a field; a document edited today and never checked against reality is zero days old on one clock and unbounded on the other). Both standards state plainly, in the standard itself, that UDS ships no checker for any of this: the minimum bar is not "build a checker" but "never let a record be silently unattended".
- **`uds audit --effects` — a gate that asks whether a component that reports success actually did anything (XSPEC-383 R8).** Shape D: reachable, called, runs, returns `{ok: true}`, touches nothing. R3's reachability gate cannot see it — reachability asks who calls a file (in-edges), and shape D lives in the out-edges (what it finally touches). Opposite direction. The root cause is that the claim and the evidence share one author: a component's return value is both "what I did" and the only proof of it, and the type system checks a shape that is *identical* for a real implementation and a fabricated one. The gate walks each declared implementation's call graph and measures whether it reaches a cross-process boundary. The boundary set is derived by walking `builtinModules` from rules about family roots (`fs`, `child_process`, `net`/`dgram`/`tls`, `http`/`https`/`http2`, `dns`, `worker_threads`) rather than hand-listed, so `fs/promises` and any future subpath need no code change, and every rule prints how many modules it expanded to — a rule matching zero has expired. Import-free globals (`fetch`, `WebSocket`, `process.dlopen`) are probed against the running runtime before being counted, and the ones absent from this Node are printed too. Families are declared **by glob** in `.uds/effect-boundary.json`, never by filename, because a filename list stops being true the moment somebody adds the seventh adapter — the exact case the gate exists for. Judgement is three-state: zero boundary hits is RED (corroborated when the family median is above zero, and reported as *uncorroborated* when the whole family is zero, since in-family comparison goes silent there); zero hits alongside an unclassifiable external dependency is UNDECIDABLE and exits 2, because calling it clean is fail-open and calling it dirty is an accusation. Per R7-b, an implementation that declares `NOT_IMPLEMENTED` and never returns a success-shaped literal is legal and needs no allowlist — the conjunction matters, since the marker alone would be the hole every hollow implementation walks through, and a file carrying both is judged RED by name. R7-c extracts domains built by string concatenation (template interpolation and `+` chains alike are folded to `*`, so `` `https://${env}.api.acme.example` `` stays decidable while `` `https://api.${tld}` `` does not) and checks them against an owned-domain list read from a pluggable source — a file, an env var, or a **command**, which is where a registrar or cloud DNS zone API goes. A declared list that cannot be read exits 2, never "the list is empty so everything is allowed". Baseline is TSV with one expiry date per row (§7.11.2), and a malformed row exits 2 because a broken baseline is not "no baseline". Building it caught a real defect in its own first self-test run: type-only imports were being followed as call-graph edges, so a hollow implementation inherited its interface file's boundary hits and went green — a type import vanishes at compile time, and counting it is a green light out of thin air. That case is now pinned by its own control arm. **What this does not prove:** that the boundary reached is the *right* one — a decorative log request passes. That is R9, which is not implemented. UDS declares no effect family of its own, so running the gate bare exits 2 by design; CI runs `--self-test`, whose seventeen arms (five probe controls, denominator, green, red, R7-b abuse, R7-b exemption, cross-file call graph, boundary canary, domain canary, unreadable list, empty collection, undecidable, broken probe) each go red on demand over a synthetic corpus written for this repo.
- **An error-exit gate adopters can run.** `templates/gates/check-error-exit.mjs` now ships in the package. `uds check` reports when a project with a `src/` directory has no such gate and never writes; `uds update` explains what it is and offers to write it, defaulting to **no**, and never overwrites an existing file. The gate allows one file to format error text and fails on the second, and exits 2 until configured.
- **`brainstorm-assistant`: the judge now needs a mind, not only a seat (XSPEC-388).** BQS D4 passes only with both an independent context and a declared model level, expressed with the vendor-neutral labels of `core/model-selection.md`; naming a concrete vendor model is a violation. CONVERGE critic briefs are rewritten as goal + constraints. No v3 mechanism is removed.
- **`llms.txt`, generated rather than hand-written.** Derived from `uds-manifest.json`, `core/`, `skills/`, `integrations/REGISTRY.json` and `cli/package.json`, with `--check` failing if the committed file differs by a byte.

### Changed

- **P1 and P3 are back in the probe set, and the reason they were cut is now a worked example of a different mistake (XSPEC-408).** Both were retired on 2026-07-23 under "baseline passed": run against Antigravity CLI 1.0.14 with no UDS installed, the model read the file before answering (P1) and committed to a recommendation of its own accord (P3). Those observations stand. What did not follow is the retirement. **Each probe measured two things — a behaviour and a declared marker — and only the behaviour was falsified.** P1's cut record says so in its own words ("what remains is that UDS asks for an explicit `[Source: <path>]` attribution while the baseline used a markdown file link"), then discards the remainder by citing §2.2 "probes assert structure, not wording". That reading cannot be right, because **P2 survives by matching four literal tags** and is the most durable probe in the set; §4's own summary had already concluded that "the durable part of the always-read tier looks like declared form and house convention, not knowledge", with two declared-form probes sitting cut a few paragraphs below it. §2.2 now states the distinction it was always meant to carry: matching a sentence the model happened to write measures luck, matching **a marker the standard requires the model to emit** measures whether the standard arrived. P1 and P3 are rewritten to ask only that narrower question and both now fail explicitly when the answer is *factually correct but unmarked*, which is the case they exist to catch. Their §2.3 entrance baselines cost no new runs: the retained 2026-07-23 transcripts were re-scored and contain `[Source:` **0 times** and `[Recommended]` **0 times**, each verified against a positive control string found in the same file by the same reader, so the zero is an absence rather than a broken search. `scripts/check-reinstated-probe-baselines.ts` re-derives both counts on every CI run — a zero read once and typed into a document is a stamp, and this repo has recorded what stamps do — with `--self-test` arms covering marker-appears (red), control-missing (undecidable, never green) and empty-transcript (undecidable), all three confirmed by mutation. **What this does not do**: raise n. It is one run per probe, one tool, one frontier model, re-read. `DELTA-TREND.md` records the re-score in its own sub-table with an explicit prohibition on adding it to the sweep above it, and the Codex row is annotated to say its 50% refers to the *retired* P1/P3 — no P1/P3 transcript was retained for Codex, so unlike Antigravity there is nothing there to re-score. The population the probes were retired *for* — the local and quantised models that adopters run under aider and continue-dev — has still never been measured, and a probe's entire value is telling you when that population stops needing the rule. Reinstating the probes puts the sensor back; it does not say what it will read.
- **Skills: every companion file is now reachable from its `SKILL.md`.** 13 companion files (3,193 lines) had nothing pointing at them and 56 more were reachable only through another companion; all 168 companions across `skills/` and the locale trees are now one hop away, each pointer saying what the file contains and when to read it. This removes a structural risk; it does not show that companions are actually read.
- **Shipped `.ai.yaml` files take `meta.updated` from the git history of their source**, and omit the field when git has no record, instead of a hand-typed date or the day the transform ran.
- **`governance-layer`: the Enforcement Reality section now also names the older Risk Acceptance fields** (`review_by`, `risks_accepted`, `gates_bypassed`) as read by no program.
- **Dependencies:** `chalk` 5.6.2 → 6.0.0, `js-yaml` 5.2.1 → 5.2.3 (CLI runtime); `eslint`, `globals`, `lint-staged`, `tsx` (development).
- **Repository-internal verification (no effect on what adopters receive):** adopter-instruction-file check that runs a real `uds init`; install-path gate across every tool in the path table; hook delivery gate that executes the installed Stop hook; content-level bundle parity; skill trigger-surface gate; shipped-stamp consistency checker; translation `source_hash` ratchet; name-mention classifier; STATUS.md contradiction gate; generated `.claude/skills` copies for this repo; the skill canary instrument (XSPEC-408 R2) and its first real runs; P7 probe cut; bump/sync roundtrip moved to every CI run; test suite no longer depends on the machine's locale.

### Deprecated

- **`contentMode: 'full'` — a second name for a mode that already existed (XSPEC-357 R7).** The manifest schema accepted it, `--content-mode` offered it, the init prompt sold it as trading context budget for compliance ("all rules embedded, ~10-15 KB, highest compliance"), and every `partial`-tier tool was auto-assigned it. None of it was implemented: the generator's only test on the mode is `=== 'minimal'`, so `full` and `index` took the same branch and wrote **byte-identical files in all 24 tool × language combinations measured**, against a control confirming the comparison detects real differences (`full` vs `minimal` differed in all 24). It could not have worked as advertised either — `.standards/` is roughly 248k tokens, far past what an integration file can carry. Adopters were choosing between two labels for one behaviour, and the label promised a difference that did not exist; the docs went as far as recommending `full` for bank core systems on regulatory grounds. **Deprecating it costs adopters nothing**: `--content-mode full` is still accepted and resolves to `index` with a notice, a manifest recording `full` is rewritten to `index` on the next read, and because the two modes already produced the same bytes **no project's generated files change**. An unrecognised mode still resolves to `index`, now with a warning naming the value; it becomes an error in 7.0.0. The `content-mode=full` scenario in `check-adopter-instruction-files.ts` — whose "byte-identical across all 8 files" finding is what led here — is removed rather than left comparing `index` against `index` and reporting identity forever.
## [6.8.0] - 2026-08-20

### Added

- **A gate that asks whether anyone can reach the code (XSPEC-383 R3).** Unit tests prove a function is correct and say nothing about whether a user can call it. On 2026-04-07 one commit added `cli/src/utils/spec-linter.js` (172 lines, 14 passing tests) and `cli/src/commands/sync.js` (133 lines) — neither was ever registered, and the same day a downstream product wired itself to `npx uds lint --json`, a command that would therefore never exist. Four and a half months, green on both sides, no connected path. `check-module-reachability.mjs` walks from package.json's `bin` and `main` along static imports, re-exports and dynamic imports, and reports what it cannot reach. BFS rather than "import count > 0", because a cluster of modules importing each other while nothing imports the cluster scores 1 apiece. It found 24 where hand-searching had found 2, including an entire nine-module `src/flow/` subsystem. Exclusions are rules with per-rule counts, never a filename whitelist; the denominator prints on success; dynamic imports that cannot be resolved statically are counted and reported rather than swallowed. Wired into CI with a baseline expiring 2026-11-19 so it does not turn red on contact, and `--self-test` runs as its own step before the check is trusted.
- **`uds lint` (XSPEC-383 R5).** Registers the command the linter has been waiting for since 2026-04-07, carrying the two checks that survive contact with real specs: dependency validity and spec size. `--json` emits `{summary, results:[{specId,status,message}]}`.

### Changed

- **Line endings are normalized before hashing (#155).** `uds check` compared on-disk bytes, so a Windows checkout with `core.autocrlf=true` turned every LF into CRLF and every hash stopped matching — and "N files modified" reads identically whether someone edited N files or the platform rewrote every newline. Normalization now happens before hashing, with git's own binary heuristic (a NUL byte in the first 8000) skipping content that must not be touched. Traversal found two further hash implementations that never went through `hasher.js` (`skills-installer.js`, `actual-state-scanner.js`); all four sites are fixed. 21 tests added, including negative controls proving that real content differences and trailing-whitespace differences are still detected on a CRLF checkout. `uds check --force` output is byte-identical before and after, so existing manifests need no re-stamp.
- **The usage-doc generator asks the CLI instead of reciting a list.** `scanCliCommands` read `cli/bin/uds.js` into a variable and then ignored it, using a hand-written array annotated "based on uds.js analysis". So every regeneration rewrote the removed `uds workflow` back into the docs, and never added `uds lint`. It now parses `uds --help`, the CLI's own authoritative answer, which covers registration styles no single-file parse can see — `mcp` is registered from another module entirely. Coverage went from 19-of-24 to 23-of-23. Parsing zero commands throws rather than emitting an empty section.

### Removed

- **`uds sync` and the AC-coverage check (XSPEC-383 R5, option E).** `sync.js` had zero consumers. The AC-coverage check was measured against 93 real specs and reports 0 of 98 acceptance criteria covered — not because they lack tests, but because it builds AC identifiers from position (`AC-1`, `AC-2`) instead of reading the ones a spec declares, then searches with a hard-coded `@` prefix. A project writing `AC-045-001` and annotating without `@` gets zero coverage regardless of how thoroughly it traces. What it reported was not "this project has no traceability" but "I looked using my own convention and found none", and those render identically. It is removed rather than left unwired: it is a correct implementation of a convention its only known consumer does not use, sitting in the tree waiting to be re-wired. Reconciling AC identifier conventions across repos is a negotiation, not a patch, and goes to its own case.
