...
## 5. DevAP / VibeOps 整合模式（Future）

### 5.1 安裝差異

`uds init` 已支援不同 AI 工具的設定。未來可透過 `--target` 參數進一步區分消費者：

| 指令 | 行為 | 安裝內容 |
|------|------|----------|
| `uds init` | 預設安裝（偵測 AI 工具） | .standards/ + AI 工具設定 |
| `uds init --target devap` | DevAP 消費者模式 | .standards/ + DevAP TestPolicy 對齊檢查 |
| `uds init --target vibeops` | VibeOps 消費者模式 | .standards/ + VibeOps Agent prompt 引用 |

### 5.2 差異化行為

**DevAP 模式**（`--target devap`）：
- 安裝核心標準（同預設）
- 額外安裝 `test-governance.ai.yaml`（DevAP TestPolicy 依賴）
- 生成 `.standards/options/testing/` 完整目錄
- 檢查 `specs/test-policy-schema.json` 與 UDS 標準的一致性

**VibeOps 模式**（`--target vibeops`）：
- 安裝核心標準（同預設）
