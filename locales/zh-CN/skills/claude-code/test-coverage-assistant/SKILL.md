...
description: |
  ...
  关键字：test coverage, completeness, dimensions, 7 dimensions, test quality, 测试覆盖, 测试完整性, 七维度。
...
> **语言**: [English](../../../../../skills/claude-code/test-coverage-assistant/SKILL.md) | 繁体中文
...
| 3 | **错误处理** | 无效输入、找不到资料 | 错误如何处理？ |
| 4 | **授权验证** | 角色权限 | 谁可以做什么？ |
| 5 | **状态变更** | 前后状态 | 状态是否正确变更？ |
| 6 | **验证逻辑** | 格式、商业规则 | 输入是否有验证？ |
| 7 | **整合测试** | 真实 DB/API 呼叫 | 查询真的有效吗？ |
...
- ❌ 多角色系统缺少授权测试
- ❌ 没有验证状态变更
- ❌ 单元测试使用万用字元但没有对应的整合测试
- ❌ 测试资料中 ID 和业务识别码使用相同值
- ❌ 测试实作细节而非行为
...
1. 检查 `CONTRIBUTING.md` 中的「测试标准」区段
2. 检查程式码库中现有的测试模式
3. 若无找到，**预设使用全部 7 个维度**
...
1. 建议：「此专案尚未设定测试完整性要求。您要自订需要哪些维度吗？」
