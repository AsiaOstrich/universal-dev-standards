...
**v4.2 的核心改动（XSPEC-388）：** D4 原本只钉住判官坐在哪里，一个字都没说它得想得动。一个独立 context 跑字面遵循型模型会逐字满足旧条文，然后照 D5–D8 逐条打勾放行——与一场真的有人反对过的评审在报告上无从分辨。v4.2 给 D4 加上**第二个轴**：评审还必须宣告其**模型层级**要求（见下方术语说明）。CONVERGE 的提示改为**目标＋约束**形状——D5–D8 是判断必须满足的判准，不是按顺序执行的步骤；既有固定句型与逐准则分数指南保留为参考格式。发散侧引用 [core/model-selection.md](../../../../core/model-selection.md)（Standard 或更高）而不另立规则。不移除任何 v3 机制。

> **术语——「模型层级」，不用「tier」：** 在本 skill 内，`tier` 已被占用两次——BQS 分级表（套用哪些维度）与 Enhanced Tier。[core/model-selection.md](../../../../core/model-selection.md) 定义的模型能力轴（该标准称 *model tier*）在本 skill 一律写作**模型层级（model level）**。模型层级是该标准的**厂商中立标签**；本 skill 绝不指名具体厂商模型（见 D4）。
...
| **D4 评估去偏** | 须 ≥3 评审 + 硬角色 Devil's Advocate + Steelman；**pass 须同时满足两轴**：(1) 独立 context，**且** (2) 以 [model-selection](../../../../core/model-selection.md) 的厂商中立标签宣告**模型层级**要求——指名具体厂商模型即违规。缺任一轴→标 `[degraded]` 不得标 pass |
...
2. **判官≠产生者：** D2/D4/D5/D7 判定须独立视角；单 context 自评只能 `[degraded]`，不得 pass。D4 另需宣告模型层级——独立性只是它两轴之一（见 D4）。
...
> **模型层级（发散侧）：** 发散提示只给目标与约束——正是 core/model-selection.md 的 [MS-004](../../../../core/model-selection.md#rules)／[Criterion 2](../../../../core/model-selection.md#criterion-2--specification-definiteness規格明確度) 的触发条件：发散以 **Standard 或更高**层级执行。不是「最高层级」——那会同时超出 MS-004 所说的与我们的证据。lagging 登记：此选择由校准回路既有的 **Diversity（D2/D3 滞后观测）** 栏验证，不另开第二套登记。
...
> **评审提示形状——目标＋约束（v4.2）：** 对面板、Devil's Advocate、Steelman 派工时——尤其派入独立 context（`--enhanced`）——提示**必须**以**目标＋其判断必须满足的约束**表达，不得写成按顺序执行的步骤清单。D5–D8 与下方的具体性要求是**判准**（「你的判断必须满足这些」），不是**步骤**（「照这个顺序做这些事」）。既有固定句型、逐准则 1–5 指南与评审权重公式**保留为参考格式**，皆非必经步骤。理由：core/model-selection.md 的 [MS-009](../../../../core/model-selection.md#rules)。

#### 步骤 3a：多评审面板

运行**三个独立评审**，各自以自己的透镜对每个想法打 1–5 分。取平均聚合以降低单评审偏误。下方加权准则定义默认透镜——是参考格式，不是评审必须逐步照走的脚本。
...
各标准指引（1–5，参考格式）：可行性（5=轻而易举 … 1=几乎不可能）；影响（5=变革性 … 1=可忽略）；工作量（5=数小时 … 1=数个季度，反向计分，工作量越低分数越高）；一致性（5=核心使命 … 1=偏离使命）。
...
约束：每个反对理由须指出**具体情境**与**具体原因**——模糊顾虑（「这可能有点难」）不接受。句型「在 [具体情境] 下，此想法会失败，因为 [具体原因]」是参考格式，不是必用句式。
...
> **BQS D4 — 判官≠产生者，两轴：** D4 唯有**两轴同时成立**才可 pass。**(1) 座位——独立性**：评审/Devil's Advocate 在独立 context 执行（`--enhanced` 隔离 agent 宿主）。**(2) 心智——宣告模型层级**：评审宣告其**模型层级**要求，以 [core/model-selection.md](../../../../core/model-selection.md) 的厂商中立标签表达；选层依据为该标准的判准（引用，不复述）。指名具体厂商模型即违规——层级标签是唯一合法的表达方式。缺任一轴→面板标 `[degraded]`、**不得标 pass**：独立 context 跑字面遵循型模型会逐字满足座位轴，成为一枚完全合规的橡皮图章——认出「不同名字下的同一件事」是打勾做不到的判断工作。baseline 单 context 下，面板是同 context 自评，标 `[degraded]`、不得 pass——诚实但非独立。不得把 baseline 跑当成 D4 pass 静默通过。

#### 步骤 3c：BQS 第 2 层——对推荐集的产物闸

收敛后，**仅对推荐集**（Agg. Score ≥ 3.5 的想法，不限笔数）套用 **D5–D8**（绝不对全部发散想法）。若无想法达 3.5，仍保留分数最高的 1 个，标记 `[未达门槛——仅供参考]`，避免报告空白。D5–D8 是**判断必须满足的判准**——不限顺序，不是程序。对推荐集中每个想法，判断必须满足：
...
> **BQS 输出新增（v4）：** **Seeds** 区（规则：被杀 ≥1 则非空）、**争议区**（高方差想法不按 mean 淘汰）、**Judgment Override** 通道（人类裁决凌驾聚合分）为 BQS 第 2/3 层所要求。Recommendations 区块为推荐集（不限笔数）中每个想法逐项记录 D5–D8 状态。自 v4.2 起，报告表头并记录评审宣告的**模型层级**（厂商中立标签）——没有它，D4 为 `[degraded]`，且宣告无处可验。
...
- 模型层级依据：[core/model-selection.md](../../../../core/model-selection.md)——D4 心智轴与发散侧层级皆以该标准的判准为准。**引用、不复述**——摘要会随来源被更正而腐坏，指针不会。
