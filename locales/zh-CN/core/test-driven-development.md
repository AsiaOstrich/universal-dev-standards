...
| **T**imely (及时) | 在生产程式码之前撰写 | 这是 TDD 的本质 |

### Uncle Bob 的 TDD 三规则

Robert C. Martin (Uncle Bob) 用三条严格的规则定义 TDD：

1. **规则一（红色规则）**：除非是为了让失败的单元测试通过，否则不允许撰写任何生产程式码。

2. **规则二（测试规则）**：不允许撰写超过足以失败的单元测试；编译失败也算失败。

3. **规则三（绿色规则）**：不允许撰写超过足以让当前失败测试通过的生产程式码。
...
| **API 设计** | ⭐⭐⭐⭐⭐ | 测试即 API 使用文件 |
| **核心业务逻辑** | ⭐⭐⭐⭐⭐ | 高价值程式码必须有测试保护 |
| **演算法实现** | ⭐⭐⭐⭐ | 边界情况多；TDD 帮助思考 |
| **重构现有程式码** | ⭐⭐⭐⭐ | 先补测试，再安全重构 |
| **UI 组件** | ⭐⭐⭐ | 部分适用；可结合 BDD |
| **探索性原型** | ⭐⭐ | TDD 可能拖慢不确定的探索 |
| **一次性脚本** | ⭐ | 成本效益比低 |
| **第三方整合** | ⭐⭐ | 难以 Mock；改用整合测试 |
...
|------|-----|-----|------|
| **焦点** | 程式码单元 | 行为 | 验收标准 |
| **语言** | 程式码 | 自然语言 (Gherkin) | 业务语言 |
| **参与者** | 开发者 | 开发者 + BA + QA | 全团队 + 利益相关者 |
| **测试层级** | 单元/整合 | 功能/场景 | 系统/验收 |
...
- **SDD**：「规格优先，程式码其次」- 定义要建构「什么」
- **TDD**：「测试优先，程式码其次」- 定义如何「验证」
...
| **整合测试** | DB 使用 Fake，外部 API 使用 Stub |
...
| **测试中的条件逻辑** | 测试程式码中的 if/else | 一个测试中有多个测试 | 分割成独立的测试 |
| **测试程式码重复** | 许多测试中相同的设置 | 维护负担 | 提取共享设置 |
| **过于特定的断言** | 断言每一个栏位 | 脆弱的测试 | 只断言相关栏位 |
| **忽略测试失败** | 跳过或注解掉失败的测试 | 隐藏的 bug | 修复或移除失败的测试 |
| **测试第三方程式码** | 测试程式库/框架行为 | 浪费努力 | 信任第三方，测试你的程式码 |
...
| 层级 | 名称 | 特征 |
|------|------|------|
| **Level 0** | 无 TDD | 程式码之后写测试（如果有的话） |
| **Level 1** | 测试优先 | 有时在程式码之前写测试 |
...
- [程式码入库标准](checkin-standards.md) - 入库要求
- [程式码审查检查清单](code-review-checklist.md) - 审查指南
