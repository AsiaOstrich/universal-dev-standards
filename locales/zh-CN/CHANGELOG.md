...
- **`contentMode: 'full'`——一个既有模式的第二个名字（XSPEC-357 R7）。** manifest schema 接受它、`--content-mode` 提供它、init 提示把它包装成「拿 context 预算换合规」的取舍（「完整嵌入所有规则、约 10-15 KB、合规率最高」），而且每一个 `partial` 层级的工具都被自动指派它。这些一项都没有被实现：生成器对模式的唯一判断是 `=== 'minimal'`，因此 `full` 与 `index` 走同一条分支，**在实测的 24 组（工具 × 语言）组合中写出逐位相同的文件**，且对照组证明该比较确实抓得到差异（`full` 对 `minimal` 在 24 组中全部不同）。它也不可能按宣称的方式运作——`.standards/` 约 248k token，远超过任何集成文件装得下的量。采用者是在两个标签之间选择同一种行为，而那个标签承诺了一个不存在的差异；文档甚至以法规理由建议银行核心系统采用 `full`。**这个 breaking change 对采用者的代价是零**：`--content-mode full` 仍然接受并解析为 `index`（附说明），manifest 中记录的 `full` 会在下次读取时改写为 `index`，而由于两个模式本来就产生相同的字节，**任何项目生成的文件都不会改变**。未知的模式现在会抛错，而不是退回默认值。`check-adopter-instruction-files.ts` 里的 `content-mode=full` 情境——正是它「8 个文件全部逐位相同」的发现促成了这次移除——一并移除，而不是留着拿 `index` 跟 `index` 比、然后永远报告「相同」。
