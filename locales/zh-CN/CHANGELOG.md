...
- **uds update 不再自动删除文件；会列出可移除的 UDS 旧文件，加 --prune 才会删，且只删 UDS 自己写过的文件。原本依赖自动清理文件的流程请加上 --prune。**（#168、#165，详见「修正」）
- **--content-mode 给了无法识别的值，现在会打印警告并照旧使用 index（6.8.0 也是生成 index 的内容）；7.0.0 起会改为直接报错。** `--content-mode full` 已弃用，会解析为 `index` 并附说明。（详见「弃用」）
- **以 `uds init -y` 安装的 Codex、OpenCode 等非 Claude 工具，技能现在会装进它们自己的目录**（例如 Codex 是 `.agents/skills/`），不再装进 `.claude/skills/`。已有安装运行 `uds update` 会自动迁移。（详见「修正」）
- **Antigravity 现在会得到 `.agents/skills/` 与 `.agents/AGENTS.md`**，以前是什么都没有、指令文件写成 `INSTRUCTIONS.md`。Antigravity 只在该目录是已登记的项目时才会读取 `.agents/`，`uds init` 会打印这个提示。
- **6.8.0 及以前 uds init --with-hooks 写进 .claude/settings.json 的 hook 是从未执行的字符串格式，uds update 不会处理 hook，重跑 uds init --with-hooks 也不会删除旧条目与 scripts/hooks/*.js。要获得可用的 hook，请重跑 uds init --with-hooks，并可手动删除旧的 .js 条目。**

### 修正

- **`uds update` 不再靠「查一张哈希表」来决定文件是谁的（XSPEC-384，issue [#165](https://github.com/AsiaOstrich/universal-dev-standards/issues/165) 与 [#168](https://github.com/AsiaOstrich/universal-dev-standards/issues/168)）。** 两位使用者对同一支命令报告了方向相反的缺陷：它删掉了使用者手写放在 `.standards/` 下的项目文件，没有警告、没有 diff、没有询问、没有备份（#168，是逐行比对 `git status` 才发现的）；同时它把 registry 已经移除的标准留在 manifest 与磁盘上，而 `upstream.version` 照样前进到「最新」（#165）。两者都源自同一个布尔值在回答两个彼此独立的问题——**这个文件是不是 UDS 写的**、**它现在还是不是现行标准**——而每个 issue 各撞上这个超载轴的一端。所有权现在改成在 UDS 写入的当下记录，存进 manifest 新增的 `provenance` 字段，并以复制动作实际返回的路径为键，而不是从来源文件名重新推算的路径（正是那个重新推算，让 `fileHashes` 里出现五条 `.standards/options/<name>.ai.yaml`，而真正的文件住在 `options/<category>/<name>.ai.yaml`）。provenance 是只增的，`fileHashes` 是当前状态；若两者同进同退，provenance 会在同一刻一起失效，也就毫无作用。删除现在必须两件事同时成立，其余每一种组合一律保留，**并打印保留的理由**。无法判定所有权时归向保留——那正是旧程序不肯选的那一边。既有安装没有 provenance，因此首次升级无从归属、也不会删任何东西；provenance 只在一次完整写入全套标准的执行结束时才建立。删除另外需要新的 `--prune` 标志，刻意不用 `--yes`：`--yes` 会被设在 CI 里，让它一并授权删除，等于正好在没有人看得到的地方把破坏性路径打开。「默认就删」的分阶段策略放在单一具名常量（`PRUNE_POLICY`），因为 XSPEC-384 §4 尚未裁决。每一份删除报告都打印分母——检查了几个、我方拥有几个、非我方几个、判不了几个、排除几个以及为什么——因为一份只列出动作对象的报告，与「它只看到这几个」无从分辨。两个 issue 都由先证明会红的测试钉住。
- **九个工具里有八个，技能被装进它们不会读的目录。** 非交互路径（`uds init -y`）下，除 Claude Code 以外每个能装技能的工具，技能都被写进 `.claude/skills/`；一个 codex 仓库执行 `--mode skills -y` 会得到零个技能、零条提示。现在「能否装技能」与「是否走 marketplace」都改为查询路径表；检测到但没有经过验证路径的工具什么都不装，并且会给出提示；`uds update` 转换旧 manifest 时会重新推导工具，不再假设 `claude-code`。Codex 那条未经验证的 `fallbackSkillsPath: '.claude/skills/'` 已移除。交互流程从未受影响。
- **Roo Code 永远装不上。** 路径表里有它，检测器里没有；它的标识符在一张表里叫 `roo`、其他地方都叫 `roo-code`，于是 `uds init` 把指令写进根目录的 `roo-code.md`。检测改用 `.roo/` 与 `.roorules`（刻意不用 `.clinerules`，那是 Cline 自己的标记）；技能也不再退回 `.claude/skills/`，因为 Roo Code 文档列出的位置里没有它。依据是 Roo Code 的文档，未在真实的 Roo Code 上实测。
- **Antigravity：UDS 六个月来什么都没为它安装，而且写的指令文件是它不会读的那一份（XSPEC-355 OQ6）。** 技能改装进 `.agents/skills/`、指令改写进 `.agents/AGENTS.md`，由工具的二进制文件、随附说明文档、以及一次带诱饵路径的实跑三方确认。它的 tier 从 `minimal` 升为 `partial`。
- **UDS 至今发布的每一个 hook 都是失效的，而采用者拿到四个坏掉的 hook。** hook 条目写成裸字符串（只有对象形式会运行）；`scripts/hooks` 没有进入 npm 包；hook 是 ESM `.js`，在 `"type": "commonjs"` 下会失败（改为 `.mjs`）；commit message 校验挂在 `UserPromptSubmit`，每输入一句话就运行一次、每次 exit 1（改挂 `PreToolUse(Bash)`，只检查真正的 `git commit -m`）。
- **`uds check` 的还原会用一份过期模板覆盖生成的集成文件。** Claude Code 那份是 140 行、最后修改于 2026-03-25 的文件。还原现在改用安装器记录的配置重新生成，受管区块外的内容不动；Antigravity 与 Gemini CLI 第一次有了还原路径，Codex 的后备来源也不再指向不存在的目录。
- **Codex／OpenCode 的 `AGENTS.md` 没有披露语句。** 2026-08-18 的披露修正只覆盖了两个生成器中的一个；选择 codex 或 opencode 时，`AGENTS.md` 走的是另一个。两种语言、两种输出格式均已修正。
- **cline、windsurf、roo-code 与 Antigravity 的集成文件只写「遵循 commit-message 规则」，没有写出规则本身。** 格式与合法类型现在直接写在文件里，与 Codex 那份原本的做法相同。

### 新增

- **`turn-completion-integrity`——agent 不得在说了下一步之后、没做就结束回合。** 由 Stop hook 在回合结束时执行，而不是写成指令。检测器附英文与 zh-TW 语言包及经过变异测试的语料；它会读取用户的最后一条消息，所以被叫停不会被当作未兑现的承诺；逐项列出「谁卡在谁身上」的清单被识别为合法的结束方式；「做完跟我说，我再验」被视为条件式。`uds init --with-hooks` 现在会在你的语言没有语言包时警告，因为在未列出的语言下，hook 装好了却不可能触发。含 zh-TW 与 zh-CN 翻译。
- **`deferred-item-exit`——六条标准会产出延后项目，而没有一条说那些项目要去哪（XSPEC-391）。** 一位采用者的一次设计工作产出了一份 ADR 与一份规格，其中带有五个标为「待决定」／「另案处理」／「不在 v1」的项目，而**五项全都没有进入任何追踪系统**。原因不是健忘。对会产出这类项目的六条标准——`adr-standards`、`spec-driven-development`、`retrospective-standards`、`feature-discovery-standards`、`code-review-checklist`、`acceptance-criteria-traceability`——grep `issue|tracker|backlog|task id|work item` 共 13 个命中，而**没有任何一个是出口**：ADR 的 `Technical Story` 是输入参照、retrospective 把 tracker 当指标来源、feature-discovery 把工单当需求输入、AC-traceability 只对外部阻塞的覆盖要求链接，而 code-review 的 8 个命中全是英文单字 "issues"，意思是「问题」。每条标准都说明如何把自己的文档写好；文档写完就是终点。新标准把缺少的那个关系陈述一次——延后项目在文档之外有一个可追溯的出口，而文档带着那个出口的识别码——六条产出端标准加上 `brainstorm-assistant` skill 各自取得**一行指向它的指针，而不是一份副本**，因为同一条规则的六份副本会往六个方向腐坏。**载体刻意不指定**：issue、受追踪的 TODO、manifest 的一列、backlog 文件里的一行都符合，判准是「这个项目离开文档了吗」，永远不是「用了哪个工具」。三个发现决定了条文。第一，出口必须在产出该文档的那次变更落地*之后*仍然解析得到——两个观察到的泄漏满足其他所有条件，仍然遗失了项目（待办写进一个会被本次合并关闭的 issue；待办写进出口的留言，被后续留言推出视线）。第二，**链接存在不等于链接有效**：回报者自己的闸门会放行他最强的那个例子——一份写着「与 issue #19 一并决定」的文档，而 #19 里根本没有这件事——所以 DEX-005/006 要求「有链接且内容已验证」与「有链接、内容未验证」永远不得印出同一个绿，因为一个被回报成通过的未知，比一个被回报成未知的未知更糟。第三，锚点是文档的**结构**（ADR 的 `Consequences`、规格的范围外章节、retrospective 的 `Action Items`），不是它的措辞：结构可以穷举，措辞清单不行，所以 DEX-008 要求任何措辞清单都要声明其涵盖率未知，DEX-009 要求任何窗口大小（「3 行以内」）都要载明来历，否则标为未校准。依 DEC-049，这条标准完全写成「必须存在的关系」，绝不写成「必须维持它的机制」——而且它在自己的本文里明白写出 **UDS 不为它提供任何闸门**，因为一条陈述了规则却没有东西运行它的标准，不承认这一点时比承认时更糟。`adr-standards` 另外补上一条它从未陈述过的边界（对它 grep `acceptance criteria|\bAC\b|驗收` 得到**零**命中，对照组 `acceptance-criteria-traceability` 为 84 个命中）：**验收标准只在一处维护——SPEC**，ADR 以链接指向它——被拷贝的清单有两个拥有者，其中一个永远不会被更新，实测后果是工作出货后 ADR 里的勾选框全部仍是未勾选，而只读规格的覆盖率工具看不到它。
- **`AUTH_SCOPE_EXTRACTION`——租户界限是一个 `WHERE` 子句，而迁移流程里没有任何东西强迫你去找它（`reverse-engineering-standards` 1.3.0，issue [#166](https://github.com/AsiaOstrich/universal-dev-standards/issues/166)）。** 一次真实的 PHP → C# 迁移中，一个以租户为界的账号列表 endpoint 在重写过程中弄丢了它的隔离谓词：依角色限缩范围被换成粗糙的「这个账号是不是任何一个已启用的管理员？」检查，于是不论是谁在问，它都返回数据库里的每一个账号。它出货了、通过了所有既有测试（那个方法一个测试都没有），最后是客户反馈「我看到了不该看到的账号信息」才浮现。`test-completeness-dimensions` 的维度 4 早就点名跨租户访问，所以缺的从来不是维度，缺的是一个在迁移当下强迫想起它的步骤。有三个性质让这一类缺陷穿过每一道闸门：那条安全关键的代码因为住在查询里而非请求／响应契约里，读起来就像众多过滤条件之一；基于形状的断言看不见范围，所以「200 加非空列表」在子句被删掉之后照样通过；而代码库别处存在一支共用 scope resolver 会被读成覆盖，同时一个新的 call site 正在不调用它地重写这个查询。新阶段与隐含规则扫描并行，机械化地推导出每一个以操作者自身身份为界的谓词——包含由 ORM global scope、repository 基类、RLS 策略、注入过滤条件的 middleware 在 SQL 之外执行的范围，因为只 grep SQL 会报告一个比实际更小的集合，而在这里更小的集合读起来恰好就像更安全的集合。每个命中都记下 `file:line`、自我绑定与触发分支；oracle 是「每个角色分支、逐 call site 的谓词等价性」，不是输出形状。RE-AUTH-001 要求一个断言另一操作者记录**不存在**的负向测试（附自我检查：在暂存副本里把谓词删掉，若测试还是通过，那它从来就不是在测范围）；RE-AUTH-002 则对任何「移植后等价物未经验证」的谓词 block cutover。
- **写下来的日期现在必须代表某件事（`tech-debt-standards` 1.1.0、`governance-layer` 1.1.0）。** 登记表模板从 1.0.0 起就要求 `Owner` 与 `Target Resolution Date`，而两份标准里没有任何一处说那个日期过了会怎样——对 `core/tech-debt-standards.md` grep `overdue|past due|expired|exceed` 零命中，而对照组 grep `Owner` 有命中，证明那个「查无」是真的、不是查询工具坏掉。更严重的是 `Registry Storage Options` 把「专用电子表格」列为完全合法：一份没有任何程序读得到的表格满足本节其余每一条要求，它没有任何地方是错的——直到日期过了，然后什么都没发生。`tech-debt-standards` 新增**到期处置**——到期时只有三种合法处置（做掉／撤销／延期并写明理由）、TD-EXP-001..006、明文禁止把到期实现成自动延期或自动关闭（两者都让时钟停止，且都不留下停了的痕迹），以及给跑不动检查的团队的合规替代方案**无人看管声明**。每个存储选项现在都带着「在什么条件下它才合法」；电子表格刻意保留，因为它常是出钱的人唯一会打开的格式。`governance-layer` 新增风险接受条款的 `review_by`（`review_by` 已过的接受不是有效条款，pipeline 必须 fail closed，与没有条款时一样）、**待裁决事项**（GOV-PD-001..004——待裁决是一种带时钟、带固定列举标记的受治理状态，不是自由书写的散文）、**汇总报告**（GOV-RPT-001..005——通过／失败／**判不了**分开打印、写出分母与其推导方式、不得合并成单一分数，因为一个「什么都不做」与「把工作做完」得分相同的指标，量的不是那份工作），以及**新鲜度指标**（GOV-FRESH-001..004——指标必须指名自己读的是编辑时间还是对账时间，两者不得共用字段；一份今天刚编辑、从未与现实对过的文件，在其中一个时钟上是零天、在另一个上是无上限）。两份标准都在标准正文里明白写出 UDS 对这些规则不提供任何检查器：最低标准不是「去建一支检查器」，而是「绝不让一条记载在无声中变成无人看管」。
- **`uds audit --effects` —— 一道去问「这个回报成功的组件，到底做了什么没有」的闸门（XSPEC-383 R8）。** 形状 D：可达、被调用、跑得动、回传 `{ok: true}`，而它什么都没碰。R3 的可达性闸门看不见它——**可达性问的是入边（谁调用它），形状 D 的病灶在出边（它最终碰到什么），方向相反**。根因是**宣称与证据共用同一个作者**：回传值既是「我做了什么」也是唯一的证据，而类型系统验的那个形状，对真实作与空壳**完全同构**。本闸门走访每个宣告的实作的调用图，量它是否触及跨进程边界。**边界集合从 runtime 自读**——由家族根名规则走访 `builtinModules` 展开（`fs`、`child_process`、`net`/`dgram`/`tls`、`http`/`https`/`http2`、`dns`、`worker_threads`），不是手打清单，因此 `fs/promises` 与未来任何子路径都不需要改代码；且**每条规则都印出自己展开了几个**，展开 0 个就是规则过期了。免 import 的全局 API（`fetch`、`WebSocket`、`process.dlopen`）先探测当前 runtime 才纳入，这个 Node 没有的那些也一并印出来。家族**用 glob 宣告**在 `.uds/effect-boundary.json`，**不用文件名清单**——文件名清单在有人加上第七个 adapter 的那一刻就不再为真，而那正是本闸门存在的理由。判定三态：零命中判 RED（家族中位数 > 0 时带「兄弟有对照组」的佐证；整个家族全为零时**组内比较集体沉默**，报告会明说这个 RED 只由绝对零判准支撑）；零命中但带着无法分类的外部依赖则是 **UNDECIDABLE 并 exit 2**——判成绿是 fail-open，判成红是诬告。依 R7-b，**宣告 `NOT_IMPLEMENTED` 且从不回传成功形状的实作是合法的，不需要白名单**——这个连言很重要：只认关键字的话，那个字就是每个空壳将来要走的洞，而两者同时出现的文件会被**指名判 RED**。R7-c 抽出字符串拼接产生的域名（模板内插与 `+` 串接一律折成 `*`，所以 `` `https://${env}.api.acme.example` `` 仍可判定，`` `https://api.${tld}` `` 不可判定），对照**来源可插拔**的持有清单：文件、环境变量，或**命令**——那正是接注册商或云账户 DNS zone API 的位置。**宣告了却读不到 → exit 2**，绝不当成「清单为空所以什么都合法」。基线是 **TSV，每列一个到期日**（§7.11.2），格式坏掉的列 exit 2，因为**坏掉的基线不是「没有基线」**。做的过程中它第一次自测就抓到自己的真缺陷：**type-only import 被当成调用图的边**，于是空壳继承了接口文件的边界命中而判绿——类型 import 编译后整行消失，把它算成命中是**凭空多出来的绿灯**。那个案例现在由一支专属对照臂钉住。**它证明不了什么**：碰到的那个边界是不是**对的**那一个——打一发装饰性 log 请求就能过关。那是 R9，本轮未做。UDS 自己没有效果家族，所以不带 config 直接跑会 exit 2，这是设计而非故障；CI 跑的是 `--self-test`，十七臂（五个探针对照组、分母、绿、红、R7-b 滥用、R7-b 豁免、跨文件调用图、边界金丝雀、域名金丝雀、读不到清单、空集合、不可判定、探针失效）在一份为本 repo 自己写的合成语料上各自证明得出来。
- **一道采用者跑得动的错误出口闸门。** `templates/gates/check-error-exit.mjs` 现在随包发布。`uds check` 会在有 `src/` 却没有这道闸门的项目上报告，但从不写入；`uds update` 会说明它是什么并提议写入，默认答案是**不要**，文件已存在时绝不覆盖。闸门允许一个文件格式化错误信息，第二个出现时才判红；配置好之前一律 exit 2。
- **`brainstorm-assistant`：判官现在需要心智，不只需要座位（XSPEC-388）。** BQS D4 必须同时具备独立 context 与声明的模型层级才算通过，模型层级只能用 `core/model-selection.md` 的厂商中立标签表达，指名具体厂商模型即违规。CONVERGE 评审提示改写为「目标＋约束」。没有移除任何 v3 机制。
- **`llms.txt` 改为生成，不再手写。** 由 `uds-manifest.json`、`core/`、`skills/`、`integrations/REGISTRY.json` 与 `cli/package.json` 推导，`--check` 在已提交的文件差一个字节时即失败。

### 变更

- **P1 与 P3 回到探针集，而它们当初被砍的理由，现在成了另一种错误的实例（XSPEC-408）。** 两者都在 2026-07-23 以「基线已通过」为由退役：在没有安装 UDS 的情况下对 Antigravity CLI 1.0.14 运行，模型会先读档再回答（P1），也会主动给出自己的推荐（P3）。这些观察仍然成立；站不住的是退役这个结论。**每一支探针量的是两件事——一个行为与一个声明的标记——而被推翻的只有行为。** P1 的退役纪录自己就写出了这一点（「剩下的是：UDS 要求明确的 `[Source: <path>]` 标注，而基线用的是 markdown 文件链接」），接着却引用 §2.2「探针断言结构，不断言用词」把剩下的部分丢掉。那个读法不可能正确，因为 **P2 正是靠比对四个字面标签存活**，而且是整组里最耐用的探针；§4 自己的总结也早已得出「always-read 层耐用的部分看起来是声明的形式与内部惯例，而不是知识」，而两支声明形式的探针就被砍在它下面几段。§2.2 现在写出它本来就该承载的区分：比对模型碰巧写出的句子，量到的是运气；比对**标准要求模型印出的标记**，量到的是标准有没有送达。P1 与 P3 改写成只问这个较窄的问题，而且两者现在都会在答案*事实正确但没有标记*时明确失败——那正是它们存在要抓的情形。它们的 §2.3 入场基线不花任何一次新运行：留存的 2026-07-23 逐字稿重新计分后，`[Source:` 出现 **0 次**、`[Recommended]` 出现 **0 次**，各自都对照同一个读取器在同一份文件里找到的阳性控制字符串，所以那个 0 是「不存在」，而不是搜索坏掉。`scripts/check-reinstated-probe-baselines.ts` 在每次 CI 运行时重新推导这两个数字——一个读过一次就打进文档的 0 是一枚戳，而这个 repo 已经记录过戳会发生什么事——其 `--self-test` 臂涵盖标记出现（红）、控制字符串缺失（判不了，绝不是绿）与空逐字稿（判不了），三者皆经突变确认。**这件事没有做到的**：提高 n。每支探针一次运行、一个工具、一个前沿模型、重读一遍。`DELTA-TREND.md` 把这次重新计分记在它自己的子表里，并明文禁止并入上方的总表；Codex 那一列加注其 50% 指的是*已退役*的 P1/P3——Codex 没有留存任何 P1/P3 逐字稿，所以不像 Antigravity 有东西可以重新计分。当初让这些探针被退役的那群对象——采用者在 aider 与 continue-dev 下运行的本机与量化模型——至今仍未被量测过，而一支探针的全部价值，就是在那群对象不再需要这条规则时告诉你。复原探针是把传感器装回去；它没有说它会读到什么。
- **技能：每一个配套文件现在都能从自己的 `SKILL.md` 一跳到达。** 原本有 13 个配套文件（3,193 行）没有任何东西指向它们，另有 56 个只能经由另一个配套文件到达；`skills/` 与各语言树的 168 个配套文件现在全部是一跳，每一条指引都写明文件内容与何时该读。这消除的是结构风险，并不证明配套文件真的会被读取。
- **发布的 `.ai.yaml` 的 `meta.updated` 改取自源文件的 git 历史**；git 查无记录时直接省略该字段，不再使用手写日期或 transform 执行当天的日期。
- **`governance-layer`：Enforcement Reality 一节现在也点名较旧的风险接受字段**（`review_by`、`risks_accepted`、`gates_bypassed`）没有任何程序在读取。
- **依赖：** `chalk` 5.6.2 → 6.0.0、`js-yaml` 5.2.1 → 5.2.3（CLI 运行时）；`eslint`、`globals`、`lint-staged`、`tsx`（开发用）。
- **仓库内部验证（不影响采用者拿到的内容）：** 实跑 `uds init` 的采用者指令文件检查；遍历路径表每个工具的安装路径闸门；实际执行已安装 Stop hook 的 hook 投递闸门；比对内容的 bundle parity；skill 触发面闸门；发布戳一致性检查器；翻译 `source_hash` 棘轮；名称提及分类器；STATUS.md 矛盾闸门；本仓库的 `.claude/skills` 副本改为生成；skill 金丝雀仪器（XSPEC-408 R2）及其首批实跑；P7 探针砍除；bump／sync 往返检查移到每次 CI；测试套件不再受机器语言环境影响。

### 弃用

- **`contentMode: 'full'`——一个既有模式的第二个名字（XSPEC-357 R7）。** manifest schema 接受它、`--content-mode` 提供它、init 提示把它包装成「拿 context 预算换合规」的取舍（「完整嵌入所有规则、约 10-15 KB、合规率最高」），而且每一个 `partial` 层级的工具都被自动指派它。这些一项都没有被实现：生成器对模式的唯一判断是 `=== 'minimal'`，因此 `full` 与 `index` 走同一条分支，**在实测的 24 组（工具 × 语言）组合中写出逐位相同的文件**，且对照组证明该比较确实抓得到差异（`full` 对 `minimal` 在 24 组中全部不同）。它也不可能按宣称的方式运作——`.standards/` 约 248k token，远超过任何集成文件装得下的量。采用者是在两个标签之间选择同一种行为，而那个标签承诺了一个不存在的差异；文档甚至以法规理由建议银行核心系统采用 `full`。**这次弃用对采用者的代价是零**：`--content-mode full` 仍然接受并解析为 `index`（附说明），manifest 中记录的 `full` 会在下次读取时改写为 `index`，而由于两个模式本来就产生相同的字节，**任何项目生成的文件都不会改变**。无法识别的模式仍然解析为 `index`，但现在会打印警告并写明那个值；7.0.0 起会改为直接报错。`check-adopter-instruction-files.ts` 里的 `content-mode=full` 情境——正是它「8 个文件全部逐位相同」的发现促成了这次移除——一并移除，而不是留着拿 `index` 跟 `index` 比、然后永远报告「相同」。
## [6.8.0] - 2026-08-20

### 新增

- **一道去问「这条 `uds <cmd>` 指示真的跑得动吗」的闸门（XSPEC-383 R4）。** 2026-08-19 查到三个顶层命令（`uds install`、`uds lint`、`uds sync`）与两个旗标（`uds check --spec-size`、`uds spec create --boost`）出现在出货的指引里而**它们都不存在**——落点包括 `quickstart.js`（它的整个工作就是「快速找到对的命令」）与 `integration-generator.js`（它把命令**写进每一个采用者的 `CLAUDE.md`／`AGENTS.md`**）。两处早已修掉（`465335e4`），而**没有任何东西在确认那个修正会维持**。`check-command-existence.mjs` 走访 `core/`、`locales/`、`ai/standards/`、`cli/src/`、`skills/`、`docs/` 与 repo 根目录的 `*.md`，抽出每一个 `uds <cmd>` 样式，用 `uds <path> --definitely-not-a-real-flag` 对本次 commit 自己的 CLI 探测——**不是 `--help`**：commander 对**未知**命令会回答通用命令表而不是报错，而那正是 `uds lint`／`uds sync` 在第一轮检查中存活的原因。
- **同一支探针对现况再跑一次，翻出第一次事故的修正从未碰到的漂移。** `uds workflow`（v6.0.0 移除）在 `docs/user/CHEATSHEET.md`、`docs/reference/FEATURE-REFERENCE.md`、`skills/workflows/README.md`（及 zh-TW／zh-CN 镜像）**被当成现行命令展示了五个月**，而 `docs/MIGRATION-v6.md` 自己就给了能抓到它的那条 grep。`uds config --lang`／`--mode` 从未存在（root `README.md`、两份 locale、`docs/reference/INTEGRATION-VERIFICATION.md`）。`uds agent install --all` 把位置参数写成旗标。`uds reverse-tdd` 是为了一段 GitHub Actions 范例**虚构出来的 CLI**。`uds check --interactive` 不存在，因为交互模式本来就是默认。**29 个发现，29 个修好，0 个残留。**
- **一道去问「有没有人到得了这段代码」的闸门（XSPEC-383 R3）。** 单元测试证明函数是对的，对「有没有使用者叫得到」一个字都没说。2026-04-07 有一次 commit 加了 `spec-linter.js`（172 行、14 个测试全过）与 `sync.js`（133 行）——**两者都从未被注册**，而同一天有下游产品把自己接上 `npx uds lint --json`，一个因此永远不会存在的命令。四个半月，两边全绿，中间没有一条通的路。`check-module-reachability.mjs` 从 package.json 的 `bin` 与 `main` 出发做 BFS，**手查找到 2 个，走访找到 24 个**，含整个九模块的 `src/flow/` 子系统。
- **`uds lint`（XSPEC-383 R5）。** 注册那个 linter 自 2026-04-07 起就在等的命令，带着两项禁得起真实 spec 考验的检查：依赖有效性与篇幅。

### 变更

- **哈希前先规范化行尾（#155）。** `uds check` 比对的是磁盘上的原始 bytes，于是 Windows 在 `core.autocrlf=true` 下的 checkout 把每个 LF 换成 CRLF，**每一个哈希都不再吻合**——而「N 个文件已修改」这句话，无论是有人改了 N 个文件还是平台改写了每一个换行，读起来完全一样。走访另外找到两套从不经过 `hasher.js` 的哈希实现，四处全修。新增 21 个测试，含负向对照：真实内容差异与尾随空白差异，在 CRLF checkout 上仍须检测得到。`uds check --force` 的输出前后逐字节相同，既有 manifest 不需要重新盖章。
- **使用说明生成器改为问 CLI，而不是背诵一份清单。** `scanCliCommands` 把 `cli/bin/uds.js` 读进一个变量然后忽略它，改用一份注记着「based on uds.js analysis」的手写数组。于是**每一次重新生成都把已移除的 `uds workflow` 写回文档**，也从来不会加入 `uds lint`。现在它解析 `uds --help`——CLI 对自己有哪些命令的权威回答，涵盖任何单文件解析看不到的注册写法（`mcp` 完全由另一个模块注册）。涵盖率从 19/24 变成 **23/23**。

### 移除

- **`uds sync` 与 AC 覆盖检查（XSPEC-383 R5，选项 E）。** `sync.js` 零消费者。AC 覆盖检查对 93 份真实 spec 测量，报告 98 条验收条件中 0 条被覆盖——**不是因为它们没有测试**，而是因为它用**位置**生成 AC 标识符（`AC-1`、`AC-2`）而不是读规格声明的那些，再用写死的 `@` 前缀去找。一个写 `AC-045-001` 并且标注不带 `@` 的项目，无论追溯做得多完整都会得到零覆盖。**它报的不是「这个项目没有追溯」，而是「我用我自己的惯例去找，找不到」，而这两句话打印出来一模一样。** 它是移除而不是留着不接：它是一个正确实现了一个唯一已知消费者并不使用的惯例的东西，躺在树上等着被人重新接上。跨 repo 的 AC 标识符惯例调解是协商，不是补丁，另案处理。
