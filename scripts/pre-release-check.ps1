...
try {
    ...
    # Step 6: Linting
    Run-Check -Step 6 -Name "Running linting" -Command "npm run lint --prefix `"$CliDir`""

    # Step 7: Tests (unless skipped)
    if ($SkipTests) {
        $Skipped++
    }
    else {
        Run-Check -Step 7 -Name "Running tests" -Command "npm test --prefix `"$CliDir`""
    }
